...
// Get user's surveys (creator only - only runs when enabled)
export function useMySurveys(options?: UseCreatorQueriesOptions) {
...
// Get survey count (creator only)
export function useSurveyCount(options?: UseCreatorQueriesOptions) {
	const enabled = options?.enabled !== false;
	return useQuery({
