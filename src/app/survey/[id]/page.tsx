...
import { ArrowLeft, ArrowRight, Send, Clock, CheckCircle, AlertCircle, Star } from "lucide-react";
...
export default function SurveyPage({ params }: SurveyPageProps) {
	...
	// Fetch survey data
	const { data: survey, isLoading, error } = useSurveyForParticipation(surveyId);
			...
			<Card className="mt-8">
				<CardContent className="flex items-center justify-between py-4">
					<div className="flex items-center space-x-4 text-sm text-muted-foreground">
						...
						{survey.endDate && (
							<span>⏰ Ends {format(new Date(survey.endDate), "MMM dd, yyyy")}</span>
						)}
					</div>
					<div className="text-xs text-muted-foreground">
						Survey by {survey.creator?.name || "Inquiro"}
					</div>
				</CardContent>
			</Card>
		</div>
