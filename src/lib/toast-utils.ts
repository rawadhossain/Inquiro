...
// Parses `{ error: string }` from Axios responses to Next API JSON errors.
export function getApiRouteErrorMessage(err: unknown, fallback: string): string {
	...
	ai: {
		generate: (promise: Promise<any>) =>
			toast.promise(promise, {
				...
				error: (err) => getApiRouteErrorMessage(err, toastMessages.ai.generatingError),
			}),
